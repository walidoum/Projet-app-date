...
var UI={
  ...
  _prefill:function(id){
    if(id==='sh-addmem'){var cats=S.categories.filter(function(c){return c.type==='souvenir'});if(!cats.length){UI.toast("Cree d'abord une categorie dans Reglages",'er');setTimeout(function(){CAT.newCat('souvenir')},300);return}UI.sel('a-cat',cats.map(function(c){return{v:c.id,l:c.icon+' '+c.name}}),cats[0].id);UI.val('a-title','');UI.val('a-addr','');UI.val('a-date',today());UI.val('a-note','');UI.val('a-fav','0');document.getElementById('a-prev').innerHTML='';document.getElementById('a-files').value='';document.getElementById('am-badge').classList.add('hidden');document.getElementById('a-gr').classList.add('hidden');if(typeof APP!=='undefined')APP._pos=null;if(typeof MEDIA!=='undefined')MEDIA.clear('addmem')}
